...
Alternatively we can use `plt.savefig('filename.png')` instead of showing it. If you do so, make sure to create the plot before you run this! `plt.show()` closes and the current plot, so calling `plt.savefig` after `plt.show` will result in an empty image. Closing the plot after showing it is useful, otherwise we would keep adding stuff to the same plot.
